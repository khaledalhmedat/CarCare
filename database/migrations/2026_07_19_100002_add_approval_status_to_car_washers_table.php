...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('car_washers', function (Blueprint $table) {
            if (!Schema::hasColumn('car_washers', 'status')) {
                $table->string('status')->default('pending')->after('is_verified');
            }
            if (!Schema::hasColumn('car_washers', 'rejection_reason')) {
                $table->text('rejection_reason')->nullable()->after('status');
            }
            if (!Schema::hasColumn('car_washers', 'approved_at')) {
                $table->timestamp('approved_at')->nullable()->after('rejection_reason');
            }
            if (!Schema::hasColumn('car_washers', 'rejected_at')) {
                $table->timestamp('rejected_at')->nullable()->after('approved_at');
            }
            if (!Schema::hasColumn('car_washers', 'suspended_at')) {
